...
#' @export
plotCTree = function(tree=sample,showLabels=TRUE,cols=NA,maxTime=NA, correct_trans, incorrect_trans)  {
  nam=tree$nam
  tree=tree$ctree
  ...
  ntot <- nsam+nh
  par(yaxt='n',bty='n')
  plot(0,0,type='l',xlim=c(min(tree[,1]),ifelse(is.na(maxTime),max(tree[,1]),maxTime)),ylim=c(0,nsam+1),xlab='',ylab='')  # This is the time axes. 
  host <- tree[ ,4] 
  if (ntot>1) {
  ...
  while (nrow(todo) > 0)  {
    ...
    if (tree[w,2] == 0 && tree[w,3] == 0)  { 
      ...
      if (showLabels) text(tree[w,1] + (max(cbind(tree[ ,1]))-min(cbind(tree[ ,1])))/20,y,nam[w], cex=0.2)
    } else if (tree[w,3] == 0)  { 
      #Transmission node 
      lines(c(x,tree[w,1]),c(y,y),col=col,lwd=2) 
      tmp = seq(tree[w,1]-0.003,tree[w,1]+0.003, by=0.001)
      round = round(tmp, digits = 3)
      # print(round)
      if (any(round %in% correct_trans)) { 
        points(tree[w,1],y,col = 'green',pch=24, bg = "green", lwd = 1)
        # print(w)
      } else if (any(round %in% incorrect_trans)){
         points(tree[w,1],y,col = 'red',pch=25, bg = "red", lwd = 1)           
       } else {
        points(tree[w,1],y,col = 'gray30',pch=22)
       }
